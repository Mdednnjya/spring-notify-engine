...
**Key finding:** Coupling a 2-second mock dispatch to the request thread
produces 80% errors and 28-second average latency under 1,000 concurrent
users. Decoupling via Redis queue drops average latency to 61ms and
eliminates errors entirely. The 202 response is immediate — the client
...
all configurations. During testing this pointed to shared PostgreSQL
write contention as the next bottleneck: all instances compete for the
same connection pool on every enqueue. The natural next step from here
is PgBouncer as a connection pooler, or a read replica to offload
idempotency key lookups from the write path.
